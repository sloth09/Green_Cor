...
**버전**: 2.3 (v2.2 Case 1/2 아키텍처 통합 + 펌핑 시간 계산 수정)
...
│   ├── export_excel.py              # Excel 내보내기
│   ├── export_docx.py               # Word 내보내기
│   └── utils.py                     # 유틸리티 함수
├── main.py                          # 단일 케이스 실행
...
```
육상 적재:                        3.33시간 (5,000 ÷ 1,500)
편도 항해 (부산항 내 이동):       1.00시간
호스 연결 및 기체 퍼징:           1.00시간
벙커링 (선박 급유):              5.00시간 (5,000 m³ 선박을 1,000 m³/h로 충전, 1회 트립)
...
**주의**: 벙커링 시간은 선박의 요구 연료량(5,000 m³)을 기준으로 계산합니다.
- 1,000 m³ 셔틀은 5회 왕복이 필요 (1회 펌핑 = 1시간)
- 5,000 m³ 셔틀은 1회 왕복이 필요 (1회 펌핑 = 5시간)

### Case 2: 여수/울산 → 부산 장거리 운송

**KEY DIFFERENCE**: Case 2에서는 한 번의 항해로 **여러 선박에 급유**합니다.

**총 사이클 시간 구성**:
```
출발지 육상 연료 탱크에서 적재:             shuttle_size / 1500 (시간)
여수/울산 → 부산 편도 항해 (15노트):       여수: 5.73시간, 울산: 1.67시간
부산항 진입:                                1시간
──────────────────────────────────────
[각 선박마다 반복 (vessels_per_trip회)]
  - 부산항 이동 (접안/계류):                1시간
  - 호스 연결 및 기체 퍼징:                 1시간
  - 벙커링 시간 (vessel당):                 5000 / pump_rate (시간) ★ (선박 요구량 기준)
  - 호스 분리 및 암모니아 퍼징:             1시간
──────────────────────────────────────
부산 → 여수/울산 편도 항해:                 여수: 5.73시간, 울산: 1.67시간
부산항 퇴출:                                1시간
──────────────────────────────────────
한 번의 완전한 사이클: 편도항해×2 + 부산항작업×(선박수) + 부산항진퇴출
```

**vessels_per_trip 자동 계산**:
```
vessels_per_trip = int(shuttle_size_m3 / bunker_volume_per_call_m3)

예시:
- 5,000 m³ shuttle ÷ 5,000 m³ = 1척 → 부산항 작업 1회
- 10,000 m³ shuttle ÷ 5,000 m³ = 2척 → 부산항 작업 2회 반복
- 25,000 m³ shuttle ÷ 5,000 m³ = 5척 → 부산항 작업 5회 반복
```

**예시 (Case 2-2: 울산, 10,000 m³ 셔틀, 1,000 m³/h 펌프)**:
```
육상 적재:           6.67시간 (10,000 ÷ 1,500)
울산 → 부산:         1.67시간
부산항 진입:         1.00시간
──────────────────────────────────────
[2척의 선박 반복 (10,000 ÷ 5,000 = 2)]
  - 선박 1:
    이동(1) + 연결(1) + 펌핑(5) + 해제(1) = 8시간
  - 선박 2:
    이동(1) + 연결(1) + 펌핑(5) + 해제(1) = 8시간
  - 소계: 16시간
──────────────────────────────────────
...
연간 최대 항해: 8,000시간 ÷ 36.13시간 = 221회
```

**예시 비교 (Case 2-2: 울산, 25,000 m³ 셔틀, 1,000 m³/h 펌프)**:
```
육상 적재:           16.67시간 (25,000 ÷ 1,500)
울산 → 부산:         1.67시간
부산항 진입:         1.00시간
──────────────────────────────────────
[5척의 선박 반복 (25,000 ÷ 5,000 = 5)]
  × 8시간/척 = 40시간
──────────────────────────────────────
부산 → 울산:         1.67시간
부산항 퇴출:         1.00시간
...
**Case 2-1 (여수) vs Case 2-2 (울산) 시간 비교** (10,000 m³ 셔틀 기준):
```
거리 차이:
  여수 → 부산: 86해리 → 5.73시간
  울산 → 부산: 25해리 → 1.67시간
  차이: 4.06시간 (절감 71%)
───────────────────────────────────────────────
Case 2-1 총 사이클: 6.67 + 5.73×2 + 1 + 16 + 1 = 36.13시간
Case 2-2 총 사이클: 6.67 + 1.67×2 + 1 + 16 + 1 = 28.34시간 (예상)

실제 차이: 약 8시간 (시간 절감 22%)
연간 항해 능력: Case 2-1: ~217회 < Case 2-2: ~282회
```
...
### Case 1: 부산항 저장소 기반
**시나리오**: 부산항에 대규모 저장 탱크가 있고, 셔틀이 탱크에서 선박으로 반복적으로 연료를 전달
...
### Case 2-1: 여수 → 부산 (장거리)
**시나리오**: 여수의 암모니아 생산 시설에서 부산으로 셔틀이 대량 운송, 1회 항해로 여러 선박에 급유
...
| **특징** | 한 번의 항해로 여러 선박 서빙 (shuttle_size > bunker_volume) |
| **셔틀 크기** | 5000 ~ 50000 m³ (10종류) |
| **저장 탱크** | 없음 (출발지에만) |
| **펌핑 시간** | bunker_volume / pump_rate (선박 요구량 기준) |

### Case 2-2: 울산 → 부산 (근거리)
**시나리오**: 울산의 암모니아 시설에서 부산으로 근거리 운송, 1회 항해로 여러 선박에 급유
...
## 코드 구조 및 모듈 설명 (Code Architecture)
...
**마지막 업데이트**: 2025-11-19 (v2.3 - Case 1/2 아키텍처 통합 + 펌핑 시간 수정 + 벙커링 가능선박 메트릭)
