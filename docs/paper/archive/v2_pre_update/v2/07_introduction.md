...
(C1) We identify distinct optimal infrastructure specifications through systematic parametric evaluation across all feasible shuttle-pump combinations: Case 1 selects a 2,500 m$^3$ shuttle at NPC $290.81M ($1.23/ton LCOA), while Cases 2-1 and 2-2 select 10,000 m$^3$ and 5,000 m$^3$ respectively, at 2.4--3.0$\times$ higher cost.
...
(C4) The sensitivity analysis reveals a differentiated cost driver hierarchy: CAPEX scaling dominates port-based storage sensitivity (62% NPC swing), while bunker volume dominates remote supply sensitivity, informing risk management strategies for each configuration.
