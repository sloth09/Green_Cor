...
# -*- coding: utf-8 -*-
"""
Runner module - Core execution logic for bunkering optimization.

This module contains reusable functions for running simulations and optimizations.
Scripts should import from here, NOT from main.py.

Functions:
    - print_cycle_time_breakdown: Print cycle time breakdown
    - run_single_scenario: Calculate cycle time for one shuttle/pump combination
    - run_annual_simulation: Simulate one year
    - run_yearly_simulation: Simulate 20 years
    - run_single_case: Run full MILP optimization
"""
...
from .export_docx import WordExporter
from .utils import calculate_vessel_growth, calculate_annual_demand
...
def print_cycle_time_breakdown(cycle_info, config, shuttle_size_cbm, pump_size_m3ph):
    ...
    Args:
        ...
        pump_size_m3ph: Pump flow rate in m3/h
    """
    # Get land pump rate
    ...
    print("\n[1 Round-Trip Voyage Breakdown]")
    print("-" * 60)

    # Common first steps
    print(f"Shore Loading:                           {cycle_info['shore_loading']:.2f}h")
    print(f"  Land Pump: {land_pump_rate:.0f} m3/h x Shuttle: {shuttle_size_cbm} m3")
    print(f"Outbound Travel:                         {cycle_info['travel_outbound']:.2f}h")
    ...
    if is_case_1:
        ...
        print(f"Connection & Purging:                    {cycle_info['setup_inbound']:.2f}h")
        print(f"Bunkering:                               {cycle_info['pumping_per_vessel']:.2f}h")
        print(f"  Pump: {pump_size_m3ph} m3/h x Shuttle Capacity: {shuttle_size_cbm} m3")
        print(f"Disconnection & Purging:                 {cycle_info['setup_outbound']:.2f}h")
    else:
        # CASE 2: One trip serves multiple vessels at destination port
        if cycle_info['port_entry'] > 0:
            print(f"Port Entry:                              {cycle_info['port_entry']:.2f}h")

        # Repeat per vessel at destination
        num_vessels_display = cycle_info['vessels_per_trip']
        if num_vessels_display > 1:
            print(f"  [Below repeated for {num_vessels_display} vessels]")

        print(f"  Docking/Movement:                      {cycle_info['movement_per_vessel']:.2f}h")
        print(f"  Connection & Purging:                  {cycle_info['setup_inbound']:.2f}h")
        print(f"  Bunkering:                             {cycle_info['pumping_per_vessel']:.2f}h")
        print(f"    Pump: {pump_size_m3ph} m3/h x Ship Fuel: {ship_fuel_per_call:.0f} m3")
        print(f"  Disconnection & Purging:               {cycle_info['setup_outbound']:.2f}h")

        if cycle_info['port_exit'] > 0:
            print(f"Port Exit:                               {cycle_info['port_exit']:.2f}h")

    print(f"Return Travel:                           {cycle_info['travel_return']:.2f}h")
    print("-" * 60)
    print(f"* Total Cycle Duration:                  {cycle_info['cycle_duration']:.2f}h")
    ...
    print(f"* Max Annual Trips (1 shuttle):          {annual_cycles_max:.0f} ({max_annual_hours:.0f}h / {cycle_info['cycle_duration']:.2f}h)")
    print("-" * 60)
...
def run_single_scenario(config, shuttle_size_cbm, pump_size_m3ph, output_path):
    ...
    Returns:
        Dictionary with time breakdown information, or None if failed
    """
    ...
    try:
        ...
        print(f"Scenario: Shuttle {shuttle_size_cbm}m3 + Pump {pump_size_m3ph}m3/h")
        print("="*60)
        ...
        print("\n[Annual Operations Metrics]")
        print("-" * 60)
        print(f"Annual Hour Limit:      8,000 hours")
        print(f"Cycle Duration:         {cycle_info['cycle_duration']:.2f} hours")
        print(f"Max Annual Trips:       {annual_cycles:.0f} ({annual_cycles * cycle_info['cycle_duration']:.0f} hours)")
        print(f"Annual Supply Capacity: {annual_supply_m3:,.0f} m3")
        print(f"* Ships Per Year:       {ships_per_year:.0f} (1 ship = {ship_fuel_per_call:.0f} m3)")
        print(f"Time Utilization:       {time_utilization:.1f}%")
        print("="*60)
...
def run_annual_simulation(config, shuttle_size_cbm, pump_size_m3ph, simulation_year, output_path):
    ...
    try:
        ...
        print(f"Shuttle: {shuttle_size_cbm} m3")
        print(f"Pump: {pump_size_m3ph} m3/h")
        print("="*60)
        ...
        annual_calls = demand_m3 / bunker_volume
        trips_per_call = cycle_info.get('trips_per_call', 1)

        # Calculate actual shuttle trips needed
        if has_storage_at_busan:
        ...
        print(f"Annual Demand:                           {demand_m3:>8,.0f} m3")
        print(f"Bunker Volume per Call:                  {bunker_volume:>8,.0f} m3")
        ...
        if export_config.get("csv", True):
            ...
            result_data = {
                "Parameter": ["Year", "Shuttle_Size_cbm", "Pump_Size_m3ph", "Required_Shuttles",
                             "Demand_m3", "Total_CAPEX_USDm", "Total_OPEX_USDm"],
                "Value": [simulation_year, shuttle_size_cbm, pump_size_m3ph, required_shuttles,
                         demand_m3, total_capex/1e6, total_opex/1e6]
            }
...
def run_yearly_simulation(config, shuttle_size_cbm, pump_size_m3ph, output_path):
    ...
    Returns:
        DataFrame with yearly results, or None if failed
    """
    output_path = Path(output_path)
    ...
    try:
        ...
        shuttle_capex = cost_calculator.calculate_shuttle_capex(shuttle_size_cbm)
        bunk_capex = cost_calculator.calculate_bunkering_capex(shuttle_size_cbm, pump_size_m3ph)
        shuttle_fixed_opex = cost_calculator.calculate_shuttle_fixed_opex(shuttle_size_cbm)
